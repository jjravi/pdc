...
iterator_init(pdcid_t objectId, pdcid_t reg_id, int blocks, struct _pdc_iterator_info *iter)
{
  ...
  if (reg_id == PDC_REGION_ALL) { /* Special handling:: We'll provide entire slices */
    ...
    if (iter->storage_order == ROW_major) {
      iter->dims[0] = 1;
      sliceCount    = region_info->dims_size[0];
      totalElements = sliceCount;
      for (i = 1; i < region_info->ndim; i++) {
        iter->dims[i] = region_info->dims_size[i];
        elementsPerSlice *= region_info->dims_size[i];
      }
      totalElements = elementsPerSlice * region_info->dims_size[0];
    }
    else {
      k             = (int)iter->ndim - 1;
      iter->dims[k] = 1;
      sliceCount    = region_info->dims_size[k];
      totalElements = sliceCount;
      for (; k >= 0; k--) {
        iter->dims[k] = region_info->dims_size[k];
        elementsPerSlice *= region_info->dims_size[k];
      }
      totalElements = elementsPerSlice * region_info->dims_size[iter->ndim - 1];
    }
    ...
    if (region_info->ndim > 1) {
      skipCount =
        ...
        element_size;
      iter->sliceResetCount = iter->slicePerBlock = region_info->dims_size[1] + 1;
      elementsPerBlock =
      ...
      if (region_info->ndim > 2) {
        ...
        iter->sliceResetCount = region_info->dims_size[1];
